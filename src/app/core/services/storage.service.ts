...
export class Storage {
  ...
  setItem(key: string, value: any): boolean {
    if (!key || !value) {
      return false; // If  key or value null or undefined
    }
  ...
  private getCookieByName(name: string) {
    ...
    const filteredCookie = cookies.split(';').filter(cookie => cookie.includes(name));
    const requestedCookieKeyValue = filteredCookie.length > 0 ? filteredCookie[0] : ':';
